...
const EMPTY_SCREENING: SiteScreening = {
  intelligence: null,
  thermal: null,
  loading: false,
  error: null,
...
function evidenceState(screening?: SiteScreening) {
  if (!screening || (!screening.completedAt && !screening.loading)) return 'pending' as const
  if (screening.loading) return 'pending' as const
  if (screening.thermal?.dataStatus === 'verified') return 'verified' as const
...
export function EnterpriseSitesPage() {
  ...
  useEffect(() => {
    ...
    api.listSites(controller.signal)
      ...
      .catch((error: unknown) => {
        ...
        setSitesError(error instanceof Error ? error.message : 'Unable to load enterprise sites.')
      })
      .finally(() => {
    ...
    // Initial route preference is resolved once.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const screenSite = useCallback(async (siteId: string, signal?: AbortSignal) => {
    setScreenings((current) => ({
      ...current,
      [siteId]: {
        ...(current[siteId] ?? EMPTY_SCREENING),
        loading: true,
        error: null,
      },
    }))

    const [intelligenceResult, thermalResult] = await Promise.allSettled([
      api.getSiteIntelligence(siteId, signal),
      api.generateThermalMap(siteId, { mode: 'site', granularityMeters: 100 }, signal),
    ])
    ...
    if (intelligenceResult.status === 'rejected' && !isAbortError(intelligenceResult.reason, signal)) {
    ...
    if (thermalResult.status === 'rejected' && !isAbortError(thermalResult.reason, signal)) {
      messages.push(thermalResult.reason instanceof Error ? thermalResult.reason.message : 'FortyGuard spatial screening could not be loaded.')
    }
    if (thermal && thermal.dataStatus !== 'verified' && thermal.message) messages.push(thermal.message)

    const next: SiteScreening = {
      intelligence,
      thermal,
      loading: false,
    ...
    setScreenings((current) => ({ ...current, [siteId]: next }))
    return next
  }, [])
  ...
  useEffect(() => {
    ...
    const screening = screenings[selectedSiteId]
    if (screening?.loading || screening?.completedAt) return

    const controller = new AbortController()
    void screenSite(selectedSiteId, controller.signal)
    return () => controller.abort()
  }, [screenSite, screenings, selectedSiteId, setSearchParams])

  const selectedSite = useMemo(() => sites.find((site) => site.id === selectedSiteId) ?? null, [selectedSiteId, sites])
  const selectedScreening = selectedSiteId ? screenings[selectedSiteId] ?? EMPTY_SCREENING : EMPTY_SCREENING
  ...
  const verifiedCount = useMemo(() => sites.filter((site) => evidenceState(screenings[site.id]) === 'verified').length, [screenings, sites])
  const highPriorityCount = useMemo(() => sites.filter((site) => {
    const risk = screenings[site.id]?.intelligence?.risk?.level
    return risk === 'high' || risk === 'extreme'
  }).length, [screenings, sites])
  const geometryReadyCount = useMemo(() => sites.filter(geometryReady).length, [sites])
  const screenedCount = useMemo(() => sites.filter((site) => Boolean(screenings[site.id]?.completedAt)).length, [screenings, sites])

  const attentionItems = useMemo(() => {
    const items: Array<{ site: Site; title: string; detail: string; tone: 'danger' | 'warning' | 'neutral' }> = []
    sites.forEach((site) => {
      const screening = screenings[site.id]
      if (!geometryReady(site)) {
        items.push({ site, title: 'Boundary incomplete', detail: 'At least three polygon points are required before FortyGuard spatial screening.', tone: 'danger' })
        return
      }
      if (!screening?.completedAt) {
        items.push({ site, title: 'Baseline screening pending', detail: 'Run portfolio screening to establish a verified spatial baseline.', tone: 'neutral' })
        return
      }
      if (screening.thermal?.dataStatus !== 'verified') {
        items.push({ site, title: 'Spatial evidence unavailable', detail: screening.thermal?.message ?? screening.error ?? 'FortyGuard returned no verified spatial cells.', tone: 'warning' })
        return
      }
      if (screening.intelligence?.fallbackActive) {
        items.push({ site, title: 'Atmospheric fallback active', detail: 'Spatial evidence remains FortyGuard-only; condition context is using a fallback provider.', tone: 'warning' })
      }
    })
  ...
  const siteCreated = (site: Site) => {
    ...
    setScreenings((current) => ({ ...current, [site.id]: EMPTY_SCREENING }))
  }

  const runPortfolioScreening = async () => {
    if (portfolioScreening || !sites.length) return
  ...
  const refreshSelected = () => {
    if (selectedSiteId) void screenSite(selectedSiteId)
  }
    ...
    <AppShell module="enterprise">
      <header className="enterprise-sites-topbar">
        <div>
          ...
          <span>Portfolio site registry, geometry readiness and evidence coverage.</span>
        </div>
      ...
      <main className="enterprise-sites-page">
        <section className="enterprise-sites-title-row">
          <div>
            <span className="enterprise-sites-eyebrow">PROPERTY REGISTRY</span>
            <h2>Sites</h2>
            <p>Manage real property boundaries, verify thermal evidence coverage, and decide which sites need deeper enterprise analysis.</p>
          </div>
          <div className="enterprise-sites-actions">
            ...
            <button type="button" className="button button--primary" onClick={() => void runPortfolioScreening()} disabled={portfolioScreening || !sites.length}>
              <RefreshCw size={16} className={portfolioScreening ? 'spin' : ''} />
              {portfolioScreening ? `Screening ${portfolioProgress.done}/${portfolioProgress.total}` : 'Run Portfolio Screening'}
            </button>
          </div>
        </section>
        ...
        {!sitesLoading && !sites.length && !sitesError && (
          <section className="enterprise-sites-empty panel">
            <span><Building2 size={28} /></span>
            <div><h2>No enterprise sites yet</h2><p>Create a property with its real Google Maps boundary. HeatShield will use that geometry for FortyGuard spatial screening instead of inventing property coverage.</p></div>
            <button type="button" className="button button--primary" onClick={() => setCreateSiteOpen(true)}><Plus size={16} /> Add First Site</button>
          </section>
        )}
        ...
        {!sitesLoading && sites.length > 0 && (
          <>
            ...
            <section className="enterprise-sites-kpis" aria-label="Portfolio site summary">
            ...
            <section className="enterprise-sites-workspace">
                  ...
                  <table className="enterprise-sites-table">
                    ...
                    <tbody>
                      {filteredSites.map((site) => {
                        ...
                        const evidence = evidenceState(screening)
                        return (
                          <tr key={site.id} className={selected ? 'selected' : ''} onClick={() => selectSite(site.id)}>
              ...
              <aside className="enterprise-sites-right-column">
                <div className="panel enterprise-sites-map-card">
                  <EnterpriseSitesMap sites={sites} selectedSiteId={selectedSiteId} thermal={selectedScreening.thermal} onSelectSite={selectSite} />
                </div>

                {selectedSite && (
                  <section className="enterprise-site-profile panel">
                    <div className="enterprise-site-profile__header">
                      <div><span className="enterprise-sites-eyebrow">SELECTED PROPERTY</span><h3>{selectedSite.name}</h3><p><MapPin size={13} /> {selectedSite.address}</p></div>
                      <span className={`enterprise-sites-risk enterprise-sites-risk--${riskTone(selectedRisk)}`}>{riskName(selectedRisk)}</span>
                    </div>
                    ...
                    <div className="enterprise-site-profile__details">
                      <div><span>Coordinates</span><strong>{selectedSite.center.lat.toFixed(4)}, {selectedSite.center.lng.toFixed(4)}</strong></div>
                      <div><span>Boundary</span><strong>{selectedSite.polygon.length} mapped points</strong></div>
                      <div><span>Operational zones</span><strong>{selectedSite.zones.length}</strong></div>
                      <div><span>Property status</span><strong className={selectedSite.status === 'active' ? 'active' : ''}>{selectedSite.status}</strong></div>
                    </div>
                    ...
                    <div className="enterprise-site-profile__actions">
                      <button type="button" className="button button--primary" onClick={refreshSelected} disabled={selectedScreening.loading}><RefreshCw size={15} className={selectedScreening.loading ? 'spin' : ''} /> {selectedScreening.loading ? 'Screening…' : 'Refresh Evidence'}</button>
                      <button type="button" className="button button--secondary" onClick={() => navigate(`/enterprise?site=${encodeURIComponent(selectedSite.id)}`)}>Open Overview <ArrowRight size={15} /></button>
                    </div>
                  </section>
                )}
              </aside>
            ...
            <section className="enterprise-sites-bottom-grid">
              <article className="enterprise-sites-attention panel">
                <div className="enterprise-sites-card-heading"><div><span className="enterprise-sites-eyebrow">DATA QUALITY</span><h3>Sites Needing Attention</h3></div><span>{attentionItems.length}</span></div>
                {attentionItems.length ? attentionItems.map((item) => (
                  <button key={`${item.site.id}-${item.title}`} type="button" onClick={() => selectSite(item.site.id)} className={`enterprise-sites-attention__row enterprise-sites-attention__row--${item.tone}`}>
                    <span>{item.tone === 'danger' ? <XCircle size={16} /> : item.tone === 'warning' ? <AlertTriangle size={16} /> : <Database size={16} />}</span>
                    <span><strong>{item.site.name}</strong><small>{item.title} · {item.detail}</small></span>
                    <ArrowRight size={15} />
                  </button>
                )) : <div className="enterprise-sites-all-clear"><ShieldCheck size={20} /><span><strong>No current evidence gaps</strong><small>All saved sites have verified spatial screening and no fallback warning.</small></span></div>}
              </article>

              <article className="enterprise-sites-coverage panel">
                <div className="enterprise-sites-card-heading"><div><span className="enterprise-sites-eyebrow">PORTFOLIO READINESS</span><h3>Evidence Coverage</h3></div><span>{sites.length ? Math.round((verifiedCount / sites.length) * 100) : 0}%</span></div>
                <div className="enterprise-sites-coverage__bar"><span style={{ width: `${sites.length ? (verifiedCount / sites.length) * 100 : 0}%` }} /></div>
                <div className="enterprise-sites-coverage__metrics">
                  <div><strong>{verifiedCount}</strong><span>Verified spatial</span></div>
                  <div><strong>{Math.max(0, screenedCount - verifiedCount)}</strong><span>Screened without cells</span></div>
                  <div><strong>{Math.max(0, sites.length - screenedCount)}</strong><span>Not screened</span></div>
                  <div><strong>{highPriorityCount}</strong><span>High priority</span></div>
                </div>
                <p>Coverage is based on actual screening responses from this session. HeatShield does not assign a portfolio heat score to unscreened sites.</p>
              </article>
            </section>
          </>
        )}
