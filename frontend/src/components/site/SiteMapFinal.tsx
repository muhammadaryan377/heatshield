...
function workerPopupHtml(worker: Worker, currentCellTemperature?: number | null) {
  ...
  return `
    <div class="worker-map-popup worker-map-popup--custom" data-worker-popup="${escapeHtml(worker.id)}">
      <button type="button" class="worker-map-popup__close" data-action="close" aria-label="Close worker details">×</button>
...
function MapFallback({ site, workers }: Pick<SiteMapProps, 'site' | 'workers'>) {
  ...
  return (
    <div className="site-map__fallback" aria-label="Site boundary preview waiting for Google Maps configuration">
      ...
      {workers.map((worker) => (
        <div key={worker.id} className={`fallback-worker fallback-worker--${worker.risk}`} style={toPercent(worker.coordinate.lat, worker.coordinate.lng)} title={`${worker.name} — ${worker.risk} risk`}>
          <UserRound size={14} />
        </div>
      ))}
...
export function SiteMap({ site, workers, temperatureC, weatherLabel }: SiteMapProps) {
  ...
  const workerOverlayRef = useRef<google.maps.OverlayView | null>(null)
  ...
  useEffect(() => {
    ...
    workerOverlayRef.current?.setMap(null)
    workerOverlayRef.current = null
  }, [site.id])
  ...
  useEffect(() => {
    ...
    loadGoogleMaps(apiKey).then((googleInstance) => {
      ...
      const closeWorkerPopup = () => {
        workerOverlayRef.current?.setMap(null)
        workerOverlayRef.current = null
      }

      const openWorkerPopup = (worker: Worker) => {
        ...
        const position = new googleInstance.maps.LatLng(worker.coordinate.lat, worker.coordinate.lng)

        class WorkerPopupOverlay extends googleInstance.maps.OverlayView {
          private element: HTMLDivElement | null = null

          onAdd() {
            const element = document.createElement('div')
            element.className = 'worker-popup-overlay-shell worker-popup-overlay-shell--above'
            element.innerHTML = workerPopupHtml(worker, workerTemperature)
            element.style.position = 'absolute'
            element.style.pointerEvents = 'auto'
            this.element = element
            this.getPanes()?.floatPane.appendChild(element)

            element.querySelector('[data-action="close"]')?.addEventListener('click', (event) => {
            ...
            element.querySelector('[data-action="adjust"]')?.addEventListener('click', () => {
          ...
          draw() {
            ...
            const width = element.offsetWidth || 292
            const height = element.offsetHeight || 256
            const sideInset = 12
            const topInset = 112
            const bottomInset = 14
            const markerGap = 26

            const left = clamp(point.x - width / 2, sideInset, Math.max(sideInset, mapWidth - width - sideInset))
            const canShowAbove = point.y - height - markerGap >= topInset
            const preferredTop = canShowAbove ? point.y - height - markerGap : point.y + markerGap
            const top = clamp(preferredTop, topInset, Math.max(topInset, mapHeight - height - bottomInset))
            const arrowX = clamp(point.x - left, 28, Math.max(28, width - 28))

            element.style.left = `${Math.round(left)}px`
            element.style.top = `${Math.round(top)}px`
            element.style.setProperty('--worker-popup-arrow-x', `${Math.round(arrowX)}px`)
            element.classList.toggle('worker-popup-overlay-shell--above', canShowAbove)
            element.classList.toggle('worker-popup-overlay-shell--below', !canShowAbove)
          }
        ...
        const overlay = new WorkerPopupOverlay()
        overlay.setMap(map)
        workerOverlayRef.current = overlay
      }
      ...
      workers.forEach((worker) => {
        ...
        marker.addListener('click', () => openWorkerPopup(worker))
        markers.push(marker)
      })

      mapClickListener = map.addListener('click', () => closeWorkerPopup())
  ...
  useEffect(() => {
    ...
    if (selection.length >= 2) {
      draftPolygonRef.current = new google.maps.Polygon({
        ...
        strokeOpacity: 1,
        fillColor: '#10a39c',
        fillOpacity: selection.length >= 3 ? 0.12 : 0,
        clickable: false,
        zIndex: 20,
      })
    }
  ...
  useEffect(() => {
    ...
    const mean = thermal.meanTemperatureC ?? ((min + max) / 2)
    const tiles = thermal.tiles
    const coveragePolygon = thermalMode === 'custom' && selection.length >= 3 ? selection : site.polygon
    ...
    class ThermalCanvasOverlay extends google.maps.OverlayView {
      ...
      onAdd() {
        ...
        canvas.className = 'thermal-canvas-overlay thermal-canvas-overlay--full-site'
      ...
      draw() {
        ...
        const beginCoveragePath = () => {
          context.beginPath()
          coveragePolygon.forEach((point, index) => {
        ...
        // Whole-site context: every point in the selected site is visibly part of
        // the thermal scene. This uses the verified site mean as a context wash;
        // provider cells remain the only spatially varying evidence rendered above it.
        context.globalAlpha = 0.26
        context.fillStyle = heatColor(mean, min, max)
        ...
        tiles.forEach((tile) => {
          ...
          context.lineWidth = tile.id === hottestTileId ? 2.2 : 0.9
          context.stroke()
        })
    ...
    if (thermalMode !== 'custom') {
      thermalClickListenerRef.current = map.addListener('click', (event: google.maps.MapMouseEvent) => {
        if (!event.latLng) return
        const point = { lat: event.latLng.lat(), lng: event.latLng.lng() }
        if (!pointInPolygon(point, coveragePolygon)) return
        ...
        const content = tile
          ? `<div class="thermal-cell-popup"><strong>${tile.temperatureC.toFixed(1)}°C</strong><span>${temperatureBand(tile.temperatureC, min, max)}</span><small>Verified FortyGuard thermal cell • clipped to site boundary</small></div>`
          : `<div class="thermal-cell-popup"><strong>${mean.toFixed(1)}°C</strong><span>Site-wide mean context</span><small>This point is covered by the site context wash; no individual provider cell covers this exact point.</small></div>`
  ...
  }, [mapVersion, selection, site.polygon, thermal, thermalMode])
  ...
  return (
    <section className={`site-map-card panel${thermalOpen ? ' site-map-card--thermal-open' : ''}`}>
      ...
      {temperatureC != null
        ? <div className="site-map-card__weather-chip"><span className="sun-symbol">☀</span><span><strong>{temperatureC.toFixed(1)}°C</strong><small>{weatherLabel ?? 'Live'}</small></span></div>
        : <div className="site-map-card__weather-chip site-map-card__weather-chip--pending"><span>Live data pending</span></div>}
      ...
      {thermalOpen && (
        <aside className="thermal-explorer" aria-label="Thermal map controls">
          ...
          <p>See the full selected site in thermal context, with verified provider cells rendered on top.</p>
          ...
          {thermal?.dataStatus === 'verified' && (
            <div className="thermal-summary">
              ...
              <small className="thermal-summary__coverage">Whole-site color wash uses the verified mean for context; only provider cells encode spatial variation.</small>
            </div>
          )}
      ...
      {thermal?.dataStatus === 'verified' && (
        <div className="thermal-legend thermal-legend--coverage">
          ...
          <small>Full-site mean context + verified cells</small>
        </div>
      )}
